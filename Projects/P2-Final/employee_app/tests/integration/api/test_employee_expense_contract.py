...
# Seeded rows from the temp_db_dsn fixture. brian is created separately by
...
class TestExpenseOwnershipContract:
    ...
    def test_cross_user_edit_is_rejected_without_persistence(
        self, live_server, employee_session, temp_db_dsn
    ):
        ...
        def read_expense():
            with psycopg.connect(temp_db_dsn) as conn:
                return conn.execute(
                    "SELECT amount, description FROM expenses WHERE id = %s",
    ...
    def test_cannot_edit_a_reviewed_expense(
        ...
        with psycopg.connect(temp_db_dsn) as conn:
            conn.execute(
                "UPDATE approvals SET status = 'approved' WHERE expenseId = %s",
                (expense_id,),
            )
            conn.commit()
...
class TestExpenseScopingContract:
    ...
    def test_reviewed_expense_moves_out_of_pending(
        ...
        with psycopg.connect(temp_db_dsn) as conn:
            conn.execute(
                "UPDATE approvals SET status = 'approved', comment = %s WHERE expenseId = %s",
