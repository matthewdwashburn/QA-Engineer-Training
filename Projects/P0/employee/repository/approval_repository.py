...
class ApprovalRepository:
    ...
    def find_expenses_with_status_for_user(self, user_id: int) -> List[tuple]:
        ...
        with self.db_connection.get_connection() as conn:
            cursor = conn.execute('''
                SELECT e.id, e.amount, e.description, e.category, e.date, a.status, a.comment, a.review_date
                FROM expenses e
            ...
            for row in cursor.fetchall():
                expense = Expense(id=row['id'], user_id=user_id,
                                  amount=row['amount'], description=row['description'], category=row['category'],
                                  date=row['date'])
