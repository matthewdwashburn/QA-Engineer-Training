...
class ExpenseRepository:
    ...
    def create_expense(self, expense):
        ...
        with self.db.get_connection() as conn:
            ...
            cursor.execute(
                "INSERT INTO expenses (userId, amount, description, category, date) VALUES (%s, %s, %s, %s, %s) RETURNING id",
                (expense.user_id, expense.amount, expense.description, expense.category, expense.date)
            )
            # Grab the new expense ID to use for the approval entry
    ...
    #     with self.db.get_connection() as conn:
    #         cursor = conn.cursor()
    #         cursor.execute("SELECT id, userId AS \"userId\", amount, description, category, date FROM expenses WHERE id = %s", (expense_id,))
    #         row = cursor.fetchone()
    #         if row:
    #             return Expense(
    #                 id=row["id"],
    ...
    def find_by_user_id(self, user_id: int) -> List[Expense]:
        ...
        with self.db.get_connection() as conn:
            cursor = conn.cursor()
            # userId is aliased in quotes because Postgres lowercases unquoted
            # output names, and the row mapping below reads row["userId"].
            cursor.execute('SELECT id, userId AS "userId", amount, category, description, date FROM expenses WHERE userId = %s', (user_id,))
    ...
    def delete_expense(self, expense_id: int) -> bool:
        ...
        with self.db.get_connection() as conn:
            ...
            cursor.execute("DELETE FROM approvals WHERE expenseId = %s", (expense_id,))
