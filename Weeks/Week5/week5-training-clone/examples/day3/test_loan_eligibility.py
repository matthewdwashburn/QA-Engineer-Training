...
def test_ineligible_income():
    assert assess_loan(age=20, income=10000,credit_score=700,employed=True) == "INELIGIBLE_INCOME"

def test_ineligible_credit():
    assert assess_loan(age=20, income=30000,credit_score=400,employed=True) == "INELIGIBLE_CREDIT"

def test_ineligible_employment():
    assert assess_loan(age=20, income=30000,credit_score=700,employed=False) == "INELIGIBLE_EMPLOYMENT"

# BVA additions: boundaries of the age range
