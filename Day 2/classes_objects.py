...
class MyNewClass:
    #this sets up the constructor for the class. There can only be one of them.
    # notice you can set the default values for parameters by declarings then within the ()
    ...
    #the __repr__ method should allow you to create a clone of the class
    def __repr__(self):
        return f"MyNewClass(self, {self.age}, {self.name})"
    
my_class = MyNewClass(name="Wil")
...
# Python supports abstract class. You make a class abstract by adding ABC inside the parentheses
class MyAbstractClass(ABC):
    ...
    def to_be_determined(self):
        pass
    # static methods are similar to class methods in that they are called by the class itself,
    # but they do not recieve the class or an instance of the class as an implicit argument. You can
    # add it but might as well use a class method at that point
    @staticmethod
    def static_method():
        return "This is my static method, notice is does not interact with the state of the class"
...
class Inheritsconstructor(InheritedConstructor):
    def __init__(self, age=0, name="default name"):
        super().__init__(name) # Super to call parent constructor
        self.age = age
