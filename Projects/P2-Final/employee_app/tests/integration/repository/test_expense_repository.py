...
class TestExpenseRepository:
    ...
    @pytest.mark.smoke
    def test_get_expenses_by_user_returns_sorted_joined_rows(self, temp_db_dsn):
        # Arrange: Create an instance of the ExpenseRepository with the temporary database path
        repo = ExpenseRepository(temp_db_dsn)
    ...
    @allure.severity(allure.severity_level.CRITICAL)
    @pytest.mark.smoke
    def test_find_expense_with_status_returns_joined_data(self, temp_db_dsn):
    ...
    @allure.severity(allure.severity_level.CRITICAL)
    @pytest.mark.smoke
    def test_update_expense_persists_amount_and_description(self, temp_db_dsn):
    ...
    @allure.severity(allure.severity_level.CRITICAL)
    @pytest.mark.smoke
    def test_delete_expense_removes_expense_and_approval(self, temp_db_dsn):
        # Arrange: Set up the ExpenseRepository instance
        repo = ExpenseRepository(temp_db_dsn)
    ...
    @allure.severity(allure.severity_level.CRITICAL)
    @pytest.mark.smoke
    def test_create_expense_inserts_expense_and_pending_approval(self, temp_db_dsn):
        repo = ExpenseRepository(temp_db_dsn)
    ...
    @allure.severity(allure.severity_level.CRITICAL)
    @pytest.mark.smoke
    def test_find_by_user_id_returns_only_target_users_rows(self, temp_db_dsn):
        repo = ExpenseRepository(temp_db_dsn)
