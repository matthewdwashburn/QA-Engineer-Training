...
class ChallengesTest {
    static Stream<Arguments> sampleTests() {
        return Stream.of(
                ...
                arguments(2,
                        new int[] { 0, 0, 8, 6, 6, 7, 8, 6, 6, 4, 7, 6, 4, 0, 1, 1, 2, 1, 2, 9, 9, 0, 3, 3, 0, 4 }),
                ...
                arguments(3,
                        new int[] { 7, 2, 0, 6, 5, 7, 3, 9, 1, 8, 4, 5, 5, 5, 8, 9, 8, 1, 9, 1, 2, 7, 2, 6, 0, 8, 0,
                                2 }),
    ...
    static Stream<Arguments> maxMatchTests() {
        return Stream.of(
            // known words segment cleanly
            arguments("thecat", List.of("the", "cat")),
            // greedy picks longest match: "the" over "t"
            arguments("thequickbrownfox", List.of("the", "quick", "brown", "fox")),
            // unknown chars fall back to single character
            arguments("xyz", List.of("x", "y", "z")),
            // single known word
            arguments("hello", List.of("hello")),
            // single unknown character
            arguments("z", List.of("z")),
            // mix of known words and unknown chars
            arguments("helloworld", List.of("hello", "world")),
            // word boundary: "and" is greedy over "an"
            arguments("and", List.of("and"))
        );
    }

    @ParameterizedTest(name = "Input: \"{0}\"")
    @MethodSource
    @DisplayName("Max Match Tests")
    void maxMatchTests(String input, List<String> expected) {
        assertEquals(expected, Challenges.maxMatch(input));
    }
}